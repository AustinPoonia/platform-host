...
/**
 * What a document may be called beyond its name. Closed, and the kernel's.
 *
 * Deliberately not the `SUFFIX` *class* that `writeCommand` validates against, and the
 * difference is the whole security property. A shim's suffix is genuinely platform
 * knowledge — `.cmd` on Windows, nothing elsewhere — so an adapter names it and the
 * kernel checks it against a shape. A document's is not knowledge anybody holds but this
 * file, and an open class would admit `.command`, `.desktop` and `.scpt`, every one of
 * which some desktop executes on open.
 *
 * The list is short because each member has to earn a line in the declaration. Adding a
 * fifth is a change to `platform:host`'s shape and gets an argument, which is the point.
 */
const DOCUMENT_SUFFIX = ['.html', '.svg', '.txt', '.csv']
...
function host (id, machine) {
   ...
   *
   * Beside `binDir` for the same reason `completions` is: everything in `binDir` is on a
   * PATH, and a document is not a command. One fixed segment rather than one per
   * anything — a document has no shell and no platform, so there is nothing to key on and
   * nothing for an artifact to name.
   */
  const documents = () => path.join(path.dirname(binDir), 'documents')
  ...
  return {
    ...
    methods: {
       ...
       * in the former — `artifact-health`'s `limits()` rows reached 526 cells, which is
       * inside the `WIDEST` clamp above and outside every real terminal. **That example was
       * repaired on 2026-08-21 and is kept here because it made the argument better than a
       * hypothetical would:** those rows are `paragraph` now and fold, so the redirect is no
       * longer the only way to read them. The reasoning is unchanged and is the point — the
       * repair was the *sending artifact's node type*, exactly as this paragraph said, and
       * reversing the precedence here would not have helped by a single cell. The next view
       * that puts prose in a `text` node reproduces it.
       *
      ...
      /**
       * Write one document for a person to open.
       *
       * ## Mode 0644, and there is no path to 0755
       *
       * `writeCommand` owns the executable case and owns the validation that goes with
       * it. A general writer that could reach `0755` would be `writeCommand` with the
       * validation removed, which is why this is a separate method with a fixed mode
       * rather than the `writeFileOfKind(kind, name, contents)` that
       * `artifact-web/README.md` predicted somebody would reach for. A `kind` argument
       * would have to carry a table of directories, modes, name classes and suffix sets,
       * keyed by a string an artifact supplies — and that table is the authority.
       *
       * ## Last-writer-wins, like `writeCompletion` and unlike `writeCommand`
       *
       * `writeCommand` refuses to clobber, because two networks both shipping `send` is a
       * real collision between two authorities and silently resolving it would decide
       * which one a person's command line means. A document has no such contest: nothing
       * dispatches on its name, and a report written twice should be the newer report.
       * The scoping does the work instead — one directory, a name class that cannot
       * express a path.
       *
       * ## The suffix decides nothing about the contents
       *
       * The bytes are written as given. A kernel that parsed them to check they matched
       * the suffix would be a second renderer nobody asked for, and it would be wrong
       * about the first artifact that wrote valid XHTML into a `.html`.
       */
      async writeDocument (name, contents, suffix) {
        const dir = documents()
        const given = suffix === undefined || suffix === null ? '.html' : String(suffix)
        if (!DOCUMENT_SUFFIX.includes(given)) {
          throw new Error(
            `${JSON.stringify(given)} is not a document suffix: one of ${DOCUMENT_SUFFIX.join(', ')}. ` +
            'The set is closed and the kernel holds it, because an open class would admit suffixes a desktop executes on open'
          )
        }
        const target = inside(dir, name, given)
        await fs.promises.mkdir(dir, { recursive: true })
        await fs.promises.writeFile(target, String(contents))
        await fs.promises.chmod(target, 0o644)
        return target
 ...
 */
module.exports = { host, COMMAND, DOCUMENT_SUFFIX, BEGIN, END }
