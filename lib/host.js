 ...
 *
 * ROADMAP §6a's fourth and last sub-wave. The kernel wires capabilities and does not
 ...
 *     `ArtifactPatform/lib/host.js` and §6a names it as a thing that must not move:
 ...
 * which is the dependency direction §6a forced and the only one available.
 *
 ...
 *
 * That is also why the list is the one piece of this file that §6a would not let move,
...
function host (id, machine) {
  ...
  return {
    ...
    methods: {
       ...
       * `ArtifactPatform/lib/host.js`'s `READABLE_ENV` and §6a would not let it move: an
       ...
       * which is the authority ROADMAP §4e says an adapter simply has. The same sentence is
 ...
 *     `ARTIFACT_COLUMNS` — is `ArtifactPatform/lib/host.js`'s and §6a would not let it
